...
export function GoalProgressCard({ goal }: { goal: Goal }) {
  ...
  return (
    <div className="rounded-lg bg-muted/30 border border-border/50 p-4 hover:border-primary/20 transition-colors">
      <div className="flex items-center justify-between mb-2">
        <div className="flex items-center gap-2">
          <span className="text-lg">{goal.icon}</span>
          <span className="text-sm font-semibold text-foreground">{goal.name}</span>
        </div>
        <span className={`text-[10px] uppercase font-semibold ${priorityColors[goal.priority]}`}>
          {goal.priority}
        </span>
      </div>
